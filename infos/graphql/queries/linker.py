...
class Linker(BalderQuery):
    ...
    def resolve(root, info, *args, name=None, id = None):
        if id:
            return models.Linker.objects.get(id=id)
        if name:
            return models.Linker.objects.get(name=name)
...
class LinkersQuery(BalderQuery):

    class Meta:
        list = True
        type = types.Linker
        filter = filters.LinkerFilter
        operation = "linkers"