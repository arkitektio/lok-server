...
class CreatePrivateFakt(BalderMutation):
    class Arguments:
        ...
        imitate = graphene.ID(description="The ID of the User to imitate (only managers can do this)", required=False)
        scopes = graphene.List(graphene.String, description="A list of potential scopes for this app", required=True)
...
class CreatePublicFakt(BalderMutation):
    class Arguments:
        ...
        kind = graphene.Argument(PublicFaktType, description="The kind of this app", required=True)
        redirect_uris = graphene.List(graphene.String, description="A list of potential redirects for this app", required=True)
        scopes = graphene.List(graphene.String, description="A list of potential scopes for this app", required=True)

    def mutate(root, info, version, identifier, kind, redirect_uris, scopes):
        # TODO: assert scopes can manage apps
        # TODO: assert permissions to create public apps
        return models.create_public_fakt(identifier, version, info.context.user, redirect_uris, scopes, kind)
